...
---
## Memolette_059_MDトグル・アイコン統一・Flutter移行決定 (2026-04-04)

### 主な変更
- MDトグル初回説明リッチダイアログ＋ON/OFFトースト
- MDテキスト+ミニトグルスイッチUI（青色ON）
- 設定テキスト変更（マークダウン切替ボタンを常時表示 / 常にマークダウンモードON）
- MDアイコンをテキスト内容自動判定に変更（isMarkdownフラグ依存廃止）
- MDマークを紫背景+白文字バッジに統一（全画面）
- アイコン並び順: ピン→MD→ロック
- ロックアイコン統一（爆速: 丸枠+縁取り、一覧: シンプル）
- フッターレイアウト改善（右端途切れ解消）
- KeyboardDismissView修正（ボタンタップ阻害解消）
- 確定ボタンをキーボード閉じるだけに統一（保存・クリア廃止）
- タイトル編集中の確定ボタン不具合修正
- 爆速モード: メモ順をフォルダ一覧と統一、日付情報表示、ピンマーク追加
- 爆速フィルタのチェックアニメーションOFF
- 最大化時カード高さ 80%→77%
